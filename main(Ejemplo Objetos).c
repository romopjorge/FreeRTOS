...
static int num_prod[] = {1, 2, 3, 4, 5};// Producer identifiers
static int num_cons[] = {1, 2};			// Consumer identifiers
...
int main(void)
{
  ...
  // Start producer tasks (wait for each to read argument)
    for (int i = 0; i < num_prod_tasks; i++) {
      printf("Producer %i\r\n", num_prod[i]);
      xTaskCreate(vProducer, task_name, 512, &(num_prod[i]), 0, NULL);
    ...
    for (int i = 0; i < num_cons_tasks; i++) {
      printf("Consumer %i\r\n", num_cons[i]);
      xTaskCreate(vConsumer, task_name, 512, &(num_cons[i]), 1, NULL);
      xSemaphoreTake(bin_sem, 0);
    }
